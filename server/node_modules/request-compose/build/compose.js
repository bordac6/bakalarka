...
var load = function load(type, middlewares) {
  return middlewares.reduce(function (obj, mw) {
    return obj[mw] = require(`./${type}/${mw}`), obj;
  }, {});
};

var Request = load('request', ['defaults', 'url', 'qs', 'form', 'json', 'body', 'auth', 'length', 'send']);

var Response = load('response', ['buffer', 'status', 'parse']);
...
var client = function client(args) {
  return compose(Request.defaults(args), function () {
    ...
    return function (_ref4) {
      var options = _ref4.options,
          body = _ref4.body;
      return args.auth ? Request.auth(args.auth)({ options, body }) : { options, body };
    };
  }(), function () {
  ...
  }(), Request.send(), Response.buffer(), Response.parse(), Response.status())();
};
